...
class AppsflyerStream(HttpStream, ABC):
    ...
    def url_base(self) -> str:
        ...
        return f"https://hq.appsflyer.com/api/{self.report_type}/export/app/{self.app_id}/"
...
class AggregateDataMixin:
    ...
    def parse_response(self, response: requests.Response, **kwargs) -> Iterable[Mapping]:
        fields = add(self.main_fields, self.additional_fields) if self.additional_fields else self.main_fields
        csv_data = map(lambda x: x.decode("utf-8"), response.iter_lines())
        reader = csv.DictReader(csv_data, fields, restkey='other')

        custom_columns = next(reader, {}).get('other', [])
        custom_columns = [re.sub(r'\W+', '_', c).strip('_').lower() for c in custom_columns]

        for record in reader:
            record['other'] = dict(zip(custom_columns, record.get('other', [])))
            yield record
...
class OrganicMixin:
    def path(
        ...
        return "organic_" + super().path(stream_state, stream_slice, next_page_token)
...
class OrganicInAppEvents(OrganicMixin, InAppEvents):
    additional_fields = fields.raw_data.additional_fields_organic_inapp
...
class OrganicUninstallEvents(OrganicMixin, UninstallEvents): ...
...
class OrganicInstalls(OrganicMixin, Installs): ...
    ...
    def check_connection(self, logger, config) -> Tuple[bool, any]:
        try:
            ...
            test_url = (
                ...
                f"https://hq.appsflyer.com/api/agg-data/export/app/{app_id}/partners_report/v5?from={dates}&to={dates}&timezone=UTC"
            )
            response = requests.get(url=test_url, headers={"Authorization": f"Bearer {api_token}"})
